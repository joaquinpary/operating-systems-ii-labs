...
void tcp_session::do_auth()
{
    ...
    [this, self](asio::error_code ec, size_t length) {
        ...
        bool valid_credentials = m_database_manager.authenticate_client(
        ...
        if (!valid_credentials) {
            ...
            if (m_auth_attempts > MAX_AUTH_ATTEMPTS) {
                ...
                server_auth_response resp = create_server_auth_response(
                    "failure",  // status
                    "",         // session_token vacío
                    "Invalid credentials"
                );
                ...
                asio::async_write(m_socket, asio::buffer(m_data, json_resp.size()), [this, self, length](asio::error_code ec, size_t) {
                    if (ec)
                    {
    ...
    m_socket.async_read_some(asio::buffer(m_data), [this, self](asio::error_code ec, size_t length) {
        if (!ec)
        {
            ...
            case CLIENT_KEEPALIVE:{
                ...
                server_emergency_alert alert = create_server_emergency_alert(
                    "infection_detected"
                );
...
void udp_server::do_send(const std::string& message, const asio::ip::udp::endpoint& endpoint, const std::string& username) {
...
void udp_server::do_auth_udp(const std::string& msg, asio::ip::udp::endpoint sender_endpoint){
    ...
    if (valid_auth) {
        ...
        m_socket.async_send_to(asio::buffer(*send_buffer, json_response.size()), sender_endpoint, [this](asio::error_code ec, size_t) {
            if (ec)
            {
                std::cerr << "Failed to send the data: " << ec.message() << "\n";
            }

...
static int handle_server_message(const message_t* msg)
{
             ...
             strcmp(msg->msg_type, SERVER_TO_HUB__INVENTORY_UPDATE) == 0)
    {
        ...
        update_inventory(msg->payload.inventory_update.items);
    }
    else if (strcmp(msg->msg_type, SERVER_TO_WAREHOUSE__ORDER_TO_DISPATCH_STOCK_TO_HUB) == 0)
    {
        // Process stock order message
        LOG_INFO_MSG("Received stock order from server");
        // Additional processing can be added here
    }
    else if (strcmp(msg->msg_type, SERVER_TO_WAREHOUSE__RESTOCK_NOTICE) == 0)
    {
        // Process restock notice message
        LOG_INFO_MSG("Received restock notice from server");
        // Additional processing can be added here
    }
    else if (strcmp(msg->msg_type, SERVER_TO_HUB__INCOMING_STOCK_NOTICE) == 0)
    {
        // Process incoming stock notice message
        LOG_INFO_MSG("Received incoming stock notice from server");
        // Additional processing can be added here
    }
    return 0;
}
...
static void do_inventory_update(void)
{
    ...
    if (create_items_message(&msg, msg_type, shared_data->client_id, SERVER, items, QUANTITY_ITEMS) == 0)
    {
        if (enqueue_pending_message(&msg) == 0)
        {
...
static void do_consume_stock(void)
{
    ...
    // Consume random amount from each item
    for (int i = 0; i < QUANTITY_ITEMS; i++)
    {
        if (shared_data->inventory_item[i].quantity > 0)
        {
            int consume_amount = get_random_consume_amount();
            int actual_consumed;

            // Don't consume more than available
            if (consume_amount > shared_data->inventory_item[i].quantity)
            {
                actual_consumed = shared_data->inventory_item[i].quantity;
                shared_data->inventory_item[i].quantity = 0;
            }
            else
            {
                actual_consumed = consume_amount;
                shared_data->inventory_item[i].quantity -= consume_amount;
            }
            ...
            if (actual_consumed > 0)
            {
                ...
                LOG_DEBUG_MSG("Consumed %d units of %s (ID: %d), remaining: %d", 
                             actual_consumed,
                             shared_data->inventory_item[i].item_name,
                             shared_data->inventory_item[i].item_id,
                             shared_data->inventory_item[i].quantity);
            }
        }
    }
    ...
    if (consumed_count > 0)
    {
        LOG_INFO_MSG("Stock consumption completed: %d items affected, %d total units consumed", 
                     consumed_count, total_consumed);
    }
    else
    {
...
static int do_check_low_stock(void)
{
    ...
    if (low_count > 0)
    {
        ...
        // Determine correct message type based on client role
        // HUB requests stock with STOCK_REQUEST, WAREHOUSE requests with REPLENISH_REQUEST
        ...
        if (create_items_message(&msg, msg_type, shared_data->client_id, SERVER, low_items, low_count) == 0)
        {
            if (enqueue_pending_message(&msg) == 0)
            {
...
static int child_logic_process(void)
{
    ...
    struct itimerspec consume_spec = {
        .it_interval = {.tv_sec = 0, .tv_nsec = 0}, // One-shot timer (no repeat)
    ...
    LOG_INFO_MSG("Timers configured: inventory=%ds, consume=random(%d-%ds), first=%ds", 
    ...
    while (!shared_data->should_exit)
    {
        // Check if inventory was updated (signal from receiver thread)
        if (shared_data->inventory_updated && !consume_timer_active)
        {
            shared_data->inventory_updated = 0; // Reset flag
            
            // Check if we now have sufficient stock to resume consumption
            sem_wait(get_inventory_sem());
            ...
            // If at least some items have stock, reactivate timer
            if (items_with_stock > 0)
            {
                int next_interval = get_random_consume_interval();
                struct itimerspec resume_spec = {
            ...
            if (FD_ISSET(consume_timer_fd, &readfds))
            {
                if (read(consume_timer_fd, &expirations, sizeof(expirations)) > 0)
                {
                    ...
                    if (all_low)
                    {
                        ...
                        struct itimerspec disable_spec = {
                            .it_interval = {.tv_sec = 0, .tv_nsec = 0},
                    ...
                    else
                    {
                        ...
                        int next_interval = get_random_consume_interval();
                        struct itimerspec new_consume_spec = {
                            .it_interval = {.tv_sec = 0, .tv_nsec = 0},
                            .it_value = {.tv_sec = next_interval, .tv_nsec = 0}};
        ...
        // ret == 0 means timeout, just check should_exit and continue
    }
...
int logic_init(client_context* ctx, const char* client_role, const char* client_id)
{
    ...
    // Initialize IPC with unique client_id for namespace isolation
